...
\Warn{Interrupt from keyboard}{
    The installed \BusyBox\ + \Linux\ system doesn't support the
    termination from keyboard (\Command{CTRL+C}); this is probably due
    to the serial interface. It's a good practice to launch programs like
    \NetCat, which waits for some event, as \emph{background process}.
